...
public class NeoTenetRedirectAdapter implements PluginTransformer {

    public static final NeoTenetRedirectAdapter INSTANCE = new NeoTenetRedirectAdapter();
    ...
    public static void scanMethod(byte[] bytes) {
        ...
        NeoTenet.LOGGER.debug(MARKER, "Scanning {}", reader.getClassName());
                    ...
                    if (METHOD_MODIFY.containsKey(key) || METHOD_REDIRECT.containsKey(key)) {
                        try {
                            ...
                            if (target != null) {
                                ...
                                if (bridge != null) {
                                    NeoTenet.LOGGER.debug(MARKER, "Creating bridge handler {}/{}{} to {}", node.name, method.name, method.desc, methodToString(target));
    ...
    @Override
    public void handleClass(ClassNode node, ClassLoaderRemapper remapper, NeoTenetRemapConfig config) {
    ...
    private static void addRule(boolean modifyArgs, Class<?> owner, String name, String handlerName, Class<?>... args) {
        ...
        Method handler = methodOf(NeoTenetReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        while (handler == null) {
            handlerArgs[0] = handlerArgs[0].getSuperclass();
            handler = methodOf(NeoTenetReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        }
        ...
        if (modifyArgs) {
            Method modifyHandler = methodOf(NeoTenetReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            if (modifyHandler == null) {
                handlerArgs[0] = original.getReturnType();
                modifyHandler = methodOf(NeoTenetReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            }
            if (modifyHandler == null) {
