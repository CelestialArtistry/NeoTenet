package org.celestial_artistry.neotenet.bukkit.remapping.patcher;
...
import org.celestial_artistry.neotenet.bukkit.remapping.GlobalClassRepo;
import org.celestial_artistry.neotenet.bukkit.remapping.PluginTransformer;
import org.celestial_artistry.neotenet.bukkit.remapping.patcher.fix.IntegratedPatcher;
...
public class NeoTenetPluginPatcher implements PluginTransformer {

    private final List<PluginPatcher> list;

    public NeoTenetPluginPatcher(List<PluginPatcher> list) {
        this.list = list;
    }
    ...
    public static List<PluginPatcher> load(List<PluginTransformer> transformerList) {
        ...
        if (pluginFolder.exists()) {
            NeoTenet.LOGGER.info("patcher.loading");
            ...
            if (files != null) {
                ...
                if (!list.isEmpty()) {
                    NeoTenet.LOGGER.info("patcher.loaded", list.size());
                }
            }
        }
        list.add(new IntegratedPatcher());
        list.sort(Comparator.comparing(PluginPatcher::priority));
        transformerList.add(new NeoTenetPluginPatcher(list));
        return list;
    }
        ...
        try (JarFile jarFile = new JarFile(file)) {
            ...
            if (jarEntry != null) {
                try (InputStream stream = jarFile.getInputStream(jarEntry)) {
                    ...
                    if (name != null) {
                        URLClassLoader loader = new URLClassLoader(new URL[]{file.toURI().toURL()}, NeoTenetPluginPatcher.class.getClassLoader());
