...
import org.celestial_artistry.neotenet.bukkit.remapping.generated.RemappingURLClassLoader;
...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    public void handleClass(ClassNode node, ClassLoaderRemapper remapper, NeoTenetRemapConfig config) {
            ...
            for (AbstractInsnNode insnNode : methodNode.instructions) {
                if (insnNode.getOpcode() == Opcodes.NEW) {
                    ...
                    if (replace != null) {
                        ...
                        NeoTenet.LOGGER.debug(MARKER, "Found new {}/{} call in {} {}", typeInsnNode.desc, ((MethodInsnNode) next).name + ((MethodInsnNode) next).desc, node.name, methodNode.name + methodNode.desc);
        ...
        if (info == null) return;
        NeoTenet.LOGGER.debug(MARKER, "Transforming classloader class {}", node.name);
    ...
    private void implementIntf(ClassNode node) {
        NeoTenet.LOGGER.debug(MARKER, "Implementing RemappingClassLoader for class {}", node.name);
        ...
        MethodNode getRemapper = new MethodNode(Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC, "getRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class)), null, null);
        {
            ...
            list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(NeoTenetRemapper.class), "createClassLoaderRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class), Type.getType(ClassLoader.class)), false));
