...
import org.celestial_artistry.neotenet.bukkit.remapping.patcher.NeoTenetPluginPatcher;
import org.celestial_artistry.neotenet.bukkit.remapping.patcher.PluginPatcher;
import org.celestial_artistry.neotenet.bukkit.remapping.source.RemapSourceHandler;
...
public class NeoTenetRemapper {
    ...
    static {
        try {
            ...
            String property = System.getProperty("neotent.remapper.dump");
            if (property != null) {
            ...
            SWITCH_TABLE_FIXER = (Function<byte[], byte[]>) Class.forName("org.celestial_artistry.neotenet.asm.SwitchTableFixer").getField("INSTANCE").get(null);
        } catch (Exception e) {
    ...
    public NeoTenetRemapper() throws Exception {
        ...
        this.toNmsMapping.loadMappings(
                new BufferedReader(new InputStreamReader(NeoTenetRemapper.class.getResourceAsStream("/mappings/bukkit_srg.srg"))),
                null, null, false
        );
        ...
        this.fromMojMapping.loadMappings(
                new BufferedReader(new InputStreamReader(NeoTenetRemapper.class.getResourceAsStream("/mappings/bukkit_moj.srg"))),
                null, null, true
        );
        BiMap<String, String> inverseClassMap = HashBiMap.create(toNmsMapping.classes).inverse();
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(NeoTenetRemapper.class.getResourceAsStream("/mappings/inheritanceMap.txt")))) {
        ...
        this.transformerList.add(NeoTenetInterfaceInvokerGen.INSTANCE);
        this.transformerList.add(NeoTenetRedirectAdapter.INSTANCE);
        this.transformerList.add(ClassLoaderAdapter.INSTANCE);
        this.patchers = NeoTenetPluginPatcher.load(this.transformerList);
