...
import org.celestial_artistry.neotenet.bukkit.remapping.generated.NeoTenetReflectionHandler;
...
public class ClassLoaderRemapper extends LenientJarRemapper {
    ...
    private static final String REPLACED_NAME = Type.getInternalName(NeoTenetReflectionHandler.class);
    ...
    public ClassLoaderRemapper(JarMapping jarMapping, JarMapping toBukkitMapping, ClassLoader classLoader) {
        ...
        this.jarMapping.setInheritanceMap(NeoTenetRemapper.INSTANCE.inheritanceMap);
    ...
    public Map.Entry<String, String> tryClimb(Map<String, String> map, String owner, String name, int access) {
        ...
        if (mapped == null && (access == -1 || (!Modifier.isPrivate(access) && !Modifier.isStatic(access)))) {
            ...
            if (NeoTenetRemapper.INSTANCE.inheritanceMap.hasParents(owner)) {
                parents = NeoTenetRemapper.INSTANCE.inheritanceMap.getParents(owner);
            } else {
                parents = GlobalClassRepo.inheritanceProvider().getParents(owner);
                NeoTenetRemapper.INSTANCE.inheritanceMap.setParents(owner, parents);
            }
    ...
    public Product2<byte[], CodeSource> remapClass(String className, Callable<byte[]> byteSource, URLConnection connection, NeoTenetRemapConfig config) throws ClassNotFoundException {
        try {
            NeoTenetClassCache.CacheSegment segment = NeoTenetClassCache.instance().makeSegment(connection);
    ...
    private byte[] remapClassFile(ClassReader reader, final ClassRepo repo) {
        ...
        NeoTenetRemapConfig config;
        if (repo instanceof ClassRepoWrapper wrapper) {
            config = wrapper.config();
        } else {
            NeoTenet.LOGGER.warn("No class remap config is provided for class {}, using PLUGIN", node.name.replace('/','.'));
            config = NeoTenetRemapConfig.PLUGIN;
        }

        for (PluginTransformer transformer : NeoTenetRemapper.INSTANCE.getTransformerList()) {
    ...
    private Class<?> generateReflectionHandler() {
        try {
            ClassNode node = MixinService.getService().getBytecodeProvider().getClassNode(Type.getInternalName(NeoTenetReflectionHandler.class));
            Preconditions.checkNotNull(node, "node");
            ClassWriter writer = new ClassWriter(0);
            String name = Type.getInternalName(NeoTenetReflectionHandler.class) + "_" + COUNTER.getAndIncrement();
    ...
    private static class NameRemapper extends Remapper {
        ...
        private static final String ORIGIN = Type.getInternalName(NeoTenetReflectionHandler.class);
    ...
    private static class PluginClassWriter extends ClassWriter {
        ...
        private String getSuper(final String typeName) {
            ...
            return NeoTenetRemapper.getNmsMapper().map(node.superName);
        }
    }
    ...
    private static byte[] dump(byte[] bytes) {
        try {
            if (NeoTenetRemapper.DUMP != null) {
                String className = new ClassReader(bytes).getClassName() + ".class";
                int index = className.lastIndexOf('/');
                if (index != -1) {
                    File file = new File(NeoTenetRemapper.DUMP, className.substring(0, index));
                    file.mkdirs();
                    Files.write(file.toPath().resolve(className.substring(index + 1)), bytes);
                } else {
                    Files.write(NeoTenetRemapper.DUMP.toPath().resolve(className), bytes);
                }
            }
        } catch (Exception e) {
